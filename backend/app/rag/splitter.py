...
class ThreeLayerSplitter(TextSplitter):
    ...
    def split(self, text: str, metadata: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        # 第一层：大段切分
        large_chunks = self._split_by_semantic_units(text, self.large_size)
        for large_idx, large_chunk in enumerate(large_chunks):
            ...
            # 第二层：中段切分
            medium_chunks = self._split_by_paragraphs(large_chunk, self.medium_size)
            for medium_idx, medium_chunk in enumerate(medium_chunks):
                all_chunks.append({
                ...
                # 第三层：小段切分
                small_chunks = self._split_by_sentences(medium_chunk, self.small_size)
                for small_idx, small_chunk in enumerate(small_chunks):
                    all_chunks.append({
                        "content": small_chunk,
                        "chunk_type": "small",
                        "parent_idx": len(all_chunks) - len(small_chunks) - 1,
                        "position": small_idx,
